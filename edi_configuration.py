...
class EdiConfiguration(models.Model):
    ...
    def generate_numero_control(self, tipo_dte, establecimiento_code=None, punto_venta_code=None):
        ...
        # Formato correcto según MH: DTE-{tipo}-{alfanumérico8}-{correlativo:015d}
        codigo_estab = (establecimiento_code or self.codigo_establecimiento or 'A001').ljust(4, '0')[:4]
        codigo_punto_venta = (punto_venta_code or 'B001').ljust(4, '0')[:4]
        # Código alfanumérico de 8 caracteres (4 estab + 4 punto venta)
    ...
    def _check_codigo_alfanumerico(self):
        """Validar que los códigos sean alfanuméricos"""
        import re
            ...
            if record.codigo_establecimiento:
                if not pattern.match(record.codigo_establecimiento):
                    raise exceptions.ValidationError(
                        _('El código de establecimiento debe contener solo letras mayúsculas (A-Z) y números (0-9)')
                    )
                if len(record.codigo_establecimiento) != 4:
                    raise exceptions.ValidationError(
                        _('El código de establecimiento debe tener exactamente 4 caracteres')
                    )
            
            if record.punto_venta:
                if not pattern.match(record.punto_venta):
                    raise exceptions.ValidationError(
                        _('El código de punto de venta debe contener solo letras mayúsculas (A-Z) y números (0-9)')
                    )
                if len(record.punto_venta) != 4:
                    raise exceptions.ValidationError(
                        _('El código de punto de venta debe tener exactamente 4 caracteres')
                    )
